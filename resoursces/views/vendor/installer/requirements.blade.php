...
@section('container')
    ...
    <ul class="list">
        ...
        <li class="list__item {{ class_exists('ZipArchive') ? 'success' : 'error' }}">
            ZipArchive
            <i class="fa fa-fw fa-{{ class_exists('ZipArchive') ? 'check-circle-o' : 'exclamation-circle' }} row-icon" aria-hidden="true"></i>
        </li>
    </ul>

    @if ( ! isset($requirements['errors']) && $phpSupportInfo['supported'] && class_exists('ZipArchive'))
        <div class="buttons">
